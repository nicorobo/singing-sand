...
pub async fn load_settings(db: &Db) -> Result<AppSettings> {
    ...
    let amplitude_scale = db.get_setting(KEY_AMPLITUDE_SCALE).await?
        .and_then(|v| v.parse::<f32>().ok()).unwrap_or(def.amplitude_scale);
    let low_gain = db.get_setting(KEY_LOW_GAIN).await?
        .and_then(|v| v.parse::<f32>().ok()).unwrap_or(def.low_gain);
    let mid_gain = db.get_setting(KEY_MID_GAIN).await?
        .and_then(|v| v.parse::<f32>().ok()).unwrap_or(def.mid_gain);
    let high_gain = db.get_setting(KEY_HIGH_GAIN).await?
        .and_then(|v| v.parse::<f32>().ok()).unwrap_or(def.high_gain);
    let display_style = db.get_setting(KEY_DISPLAY_STYLE).await?
        .and_then(|v| parse_display_style(&v)).unwrap_or(def.display_style);
    let color_scheme = db.get_setting(KEY_COLOR_SCHEME).await?
        .and_then(|v| parse_color_scheme(&v)).unwrap_or(def.color_scheme);
    let normalize_mode = db.get_setting(KEY_NORMALIZE_MODE).await?
        .and_then(|v| parse_normalize_mode(&v)).unwrap_or(def.normalize_mode);
    let gamma = db.get_setting(KEY_GAMMA).await?
        .and_then(|v| v.parse::<f32>().ok()).unwrap_or(def.gamma);
    let noise_floor = db.get_setting(KEY_NOISE_FLOOR).await?
        .and_then(|v| v.parse::<f32>().ok()).unwrap_or(def.noise_floor);
    let smoothing = db.get_setting(KEY_SMOOTHING).await?
        .and_then(|v| v.parse::<u8>().ok()).unwrap_or(def.smoothing);
    Ok(AppSettings {
        waveform: WaveformRenderSettings {
            amplitude_scale, low_gain, mid_gain, high_gain,
            display_style, color_scheme, normalize_mode,
            gamma, noise_floor, smoothing,
        },
    })
}
...
fn parse_display_style(s: &str) -> Option<DisplayStyle> {
    match s { "mirrored" => Some(DisplayStyle::Mirrored), "tophalf" => Some(DisplayStyle::TopHalf), _ => None }
}

pub fn display_style_to_str(s: DisplayStyle) -> &'static str {
    match s { DisplayStyle::Mirrored => "mirrored", DisplayStyle::TopHalf => "tophalf" }
}

fn parse_normalize_mode(s: &str) -> Option<NormalizeMode> {
    match s { "none" => Some(NormalizeMode::None), "perband" => Some(NormalizeMode::PerBand), "global" => Some(NormalizeMode::Global), _ => None }
}

pub fn normalize_mode_to_str(m: NormalizeMode) -> &'static str {
    match m { NormalizeMode::None => "none", NormalizeMode::PerBand => "perband", NormalizeMode::Global => "global" }
}
...
pub fn color_scheme_to_str(s: ColorScheme) -> &'static str {
    match s {
